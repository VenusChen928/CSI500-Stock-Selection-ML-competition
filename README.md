...
baseline_xgb:            mean excess +0.600%, min -1.221%, 5 negative windows / 12
```
...
`stage2_report/backtests/` contains the per-window portfolio CSVs generated by
this final 12-window self-test, plus companion meta CSVs for the adaptive
route.  Rerunning the command above refreshes those portfolio records and the
retained summary/detail evidence files.
